...
const Layout: React.FC<LayoutProps> = ({ children, currentUser, setCurrentPage, currentPage, switchUser }) => {
  
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { id: 'ideas', label: 'Baza Pomysłów', icon: Lightbulb },
    { id: 'repository', label: 'Repozytorium', icon: Library },
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs text-gray-400">v1.0.0 • Connected to Firestore (Simulated)</span>
          </div>
        </header>
